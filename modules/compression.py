...
class rgb_to_ycbcr_jpeg(nn.Module):
    ...
    def __init__(self):
        ...
        self.shift = nn.Parameter(torch.tensor([0., mean_tif_8, mean_tif_8]))
        #
...
class y_quantize(nn.Module):
    ...
    Output:
        image(tensor): batch x height x width
    """
    def __init__(self, rounding, tables):
        super(y_quantize, self).__init__()
        self.rounding = rounding
        self.y_table = tables[0]

    def forward(self, image):
        image = image.float() / (self.y_table)
...
class compress_jpeg(nn.Module):
    ...
    def __init__(self,rounding,tables):
        ...
        self.c_quantize = c_quantize(rounding=rounding, tables=tables)
        self.y_quantize = y_quantize(rounding=rounding, tables=tables)

    def forward(self, image):
        y, cb, cr = self.l1(image*(2**(8)-1))
